...
export class TableLayout extends go.Layout {
  ...
  * @return {EnumValue}
  */
	public getEffectiveTableStretch(child: go.Part, row: go.RowColumnDefinition, col: go.RowColumnDefinition): go.EnumValue {
  ...
  */
	public measureTable(width: number, height: number, children: go.List<go.Part>, union: go.Size, minw: number, minh: number): Array<Array<any>> {
		...
		var rowcol: Array<Array<any>> = []; // saved (so no temp array) starts as an array of rows, will end up [row][col][cell]
		for (var i = 0; i < l; i++) {
    ...
    this.beforeMeasure(children, rowcol);
		...
		for (var i = 0; i < lrow; i++) {
			...
			for (var j = 0; j < lcol; j++) {
				...
				for (var k = 0; k < len; k++) {
					...
					var marg = child.margin as go.Margin;
					...
					if (child.rowSpan === 1 && (realheight || stretch === go.GraphObject.None || stretch === go.GraphObject.Horizontal)) {
						var myDef = this.getRowDefinition(i);
						amt = Math.max(mheight - myDef.actual, 0);
						if (amt > rowleft) amt = rowleft;
						myDef.actual = myDef.actual + amt;
					...
					if (child.columnSpan === 1 && (realwidth || stretch === go.GraphObject.None || stretch === go.GraphObject.Vertical)) {
						var myDef = this.getColumnDefinition(j);
						amt = Math.max(mwidth - myDef.actual, 0);
		...
		for (var i = 0; i < l; i++) {
			...
			if (colHerald.actual === 0 && (<any>nosizeCols)[child.column] !== undefined) {
				(<any>nosizeCols)[child.column] = Math.max(mb.width + margw, (<any>nosizeCols)[child.column]);
			} else {
				(<any>nosizeCols)[child.column] = null; // obey the column herald
			}
			if (rowHerald.actual === 0 && (<any>nosizeRows)[child.row] !== undefined) {
				(<any>nosizeRows)[child.row] = Math.max(mb.height + margh, (<any>nosizeRows)[child.row]);
			} else {
				(<any>nosizeRows)[child.row] = null; // obey the row herald
			}
		}
		...
		for (var i = 0; i < l; i++) {
			...
			if (isFinite(colHerald.width)) {
				...
				if (isFinite(colleft) && (<any>nosizeCols)[child.column] !== null) {
					if (desiredColTotal === 0) w = colHerald.actual + colleft;
					else w = /*colHerald.actual +*/ (((<any>nosizeCols)[child.column] / desiredColTotal) * originalcolleft);
				} else {
					// Only use colHerald.actual if it was nonzero before this loop
					if ((<any>nosizeCols)[child.column] !== null) w = colleft;
			...
			if (isFinite(rowHerald.height)) {
				...
				if (isFinite(rowleft) && (<any>nosizeRows)[child.row] !== null) {
					if (desiredRowTotal === 0) h = rowHerald.actual + rowleft;
					else h = /*rowHerald.actual +*/ (((<any>nosizeRows)[child.row] / desiredRowTotal) * originalrowleft);
				} else {
					// Only use rowHerald.actual if it was nonzero before this loop
					if ((<any>nosizeRows)[child.row] !== null) h = rowleft;
  ...
  * @this {TableLayout}
  */
	public arrangeTable(children: go.List<go.Part>, union: go.Size, rowcol: Array<Array<any>>) {
		var l = children.length;
		...
		var lrow = (<any>rowcol).length; //number of rows
		var lcol = 0;
		for (var i = 0; i < lrow; i++) {
			if (!(<any>rowcol)[i]) continue;
			lcol = Math.max(lcol, (<any>rowcol)[i].length); // column length in this row
		}
